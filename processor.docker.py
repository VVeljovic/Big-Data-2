...
from pyspark.sql.functions import from_csv, col, count, avg, stddev, window, sum, min, max, to_json, struct, lit
import sys
...
spark = SparkSession.builder.appName("Big-Data-2").master("local[*]").getOrCreate()
...
text_column = "Number of riders where " + filter_column + " > " + str(filter_value)

filtered_df = filtered_df.withColumn("condition", lit(text_column)) \

non_empty_df_filtered = filtered_df.filter(col("count") > 0)

json_df_filtered = non_empty_df_filtered.select(to_json(struct("*")).alias("value"))

query_filtered = json_df_filtered.writeStream \
        .outputMode("update") \
        .format("kafka") \
        .option("kafka.bootstrap.servers", "localhost:9092") \
        .option("topic", "filtered_results") \
        .option("checkpointLocation", "/tmp/spark_checkpoints/filtered_results") \
        .start()

query_filtered.awaitTermination()

agg_df = parsed_df.withWatermark("tpep_pickup_datetime", "30 minutes") \
